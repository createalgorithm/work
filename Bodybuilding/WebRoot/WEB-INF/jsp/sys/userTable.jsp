                    ...
                    <marquee>
                             <table>
                                    ...
                                    <c:forEach var="list" items="${findUserList}">
                                        	<td>恭喜&nbsp;“${list.name}”&nbsp;预定了${list.uteach}号婚纱套系。&nbsp;&nbsp;&nbsp;</td>
                                    </c:forEach>
                                    </tbody>
                                </table>                                                    
                    </marquee>
...
<!--                                     	<th>所选教练</th> -->
                                    ...
                                    <c:forEach var="list" items="${findUserList}">
                                        <tr>
                                        	...
                                        	<td>${list.tel}</td>
                                        	<td>${list.address}</td>
<!--                                         	<td>${list.uclass}婚纱套系</td> -->
                                        	<td>${list.uteach}号婚纱套系</td>
                                        	<td>${list.adddate}</td>
                                        	<td>
                                        	<button class="btn btn-primary btn-fill " onclick="update(${list.id},'${list.account}','${list.name}','${list.pwd}',${list.sex},${list.age},'${list.tel}','${list.address}','${list.uteach}')">编辑</button>
...
function update(id,account,name,pwd,sex,age,tel,address,uteach){
	parent.layer.open({
	    type: 2,
	    shadeClose: true,
	    shade: 0.8,
	    area: ['450px', '75%'],
	    content: 'upUser?id='+id+'&account='+account+'&name='+name+'&pwd='+pwd+'&sex='+sex+'&age='+age+'&tel='+tel+'&address='+address+'&uteach='+uteach,
	    end: function () {
	       window.location.href='userTable';
	    }
	});
